...
export async function POST(request: Request) {
  ...
  try {
    ...
    await newOrder.save();
    return NextResponse.json({ newOrder, message: "Order successful" }, { status: 200 });

  } catch (error) {
    console.error(error);
    return NextResponse.json({ message: "Order not successful", error: error}, { status: 500 });
  }
}
...
export async function GET(){
     await connectToDataBase()
     try {
       const order = await Order.find().populate({path:'userId',select:['firstName','lastName','email']}).populate({path:'productId',select:['name','prise','brandName','image','description','size','option']})
